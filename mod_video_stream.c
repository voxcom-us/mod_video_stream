/*
 * mod_video_stream FreeSWITCH module to stream audio to websocket and receive response
 */
#include "mod_video_stream.h"
#include "video_streamer_glue.h"

SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_video_stream_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_video_stream_runtime);
SWITCH_MODULE_LOAD_FUNCTION(mod_video_stream_load);

SWITCH_MODULE_DEFINITION(mod_video_stream, mod_video_stream_load, mod_video_stream_shutdown, NULL /*mod_video_stream_runtime*/);
...
static void responseHandler(switch_core_session_t *session, const char *eventName, const char *json)
{
    ...
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_video_stream: failed to create event for %s\n", eventName);
        return;
    }
                                     ...
                                     char *metadata)
{
    ...
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_video_stream: bug already attached!\n");
        return SWITCH_STATUS_FALSE;
    }

    if (switch_channel_answer(channel) != SWITCH_STATUS_SUCCESS)
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_video_stream: channel must have reached pre-answer status before calling start!\n");
        return SWITCH_STATUS_FALSE;
    }
    ...
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_video_stream: no read codec available, aborting start\n");
        return SWITCH_STATUS_FALSE;
    }
    ...
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Error initializing mod_video_stream session.\n");
        return SWITCH_STATUS_FALSE;
    }
...
static switch_status_t do_stop(switch_core_session_t *session, char *text)
{
    ...
    if (text)
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_video_stream: stop w/ final text %s\n", text);
    }
    else
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_video_stream: stop\n");
    }
...
static switch_status_t do_pauseresume(switch_core_session_t *session, int pause)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_video_stream: %s\n", pause ? "pause" : "resume");
...
static switch_status_t send_text(switch_core_session_t *session, char *text)
{
    ...
    if (bug)
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_video_stream: sending text: %s.\n", text);
        status = stream_session_send_text(session, text);
    }
    else
    {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_video_stream: no bug, failed sending text: %s.\n", text);
    }
    return status;
}
...
SWITCH_STANDARD_API(stream_function)
{
    ...
    assert(cmd);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "mod_video_stream cmd: %s\n", cmd ? cmd : "");
    ...
    else
    {
        ...
        if ((lsession = switch_core_session_locate(argv[0])))
        {
            ...
            {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "unsupported mod_video_stream cmd: %s\n", argv[1]);
            }
            switch_core_session_rwunlock(lsession);
        }
        else
...
SWITCH_MODULE_LOAD_FUNCTION(mod_video_stream_load)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_video_stream API loading..\n");
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't register an event subclass for mod_video_stream API.\n");
        return SWITCH_STATUS_TERM;
    }
    SWITCH_ADD_API(api_interface, "uuid_video_stream", "video_stream API", stream_function, STREAM_API_SYNTAX);
    switch_console_set_complete("add uuid_video_stream ::console::list_uuid start wss-url metadata");
    switch_console_set_complete("add uuid_video_stream ::console::list_uuid start wss-url");
    switch_console_set_complete("add uuid_video_stream ::console::list_uuid stop");
    switch_console_set_complete("add uuid_video_stream ::console::list_uuid pause");
    switch_console_set_complete("add uuid_video_stream ::console::list_uuid resume");
    switch_console_set_complete("add uuid_video_stream ::console::list_uuid send_text");

    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_video_stream API successfully loaded\n");
...
/*
  ...
  Macro expands to: switch_status_t mod_video_stream_shutdown() */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_video_stream_shutdown)
{
